...
export default function EditAppVersionPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params)
  ...
  const loadVersion = useCallback(async () => {
    try {
      ...
      setFormData({
        ...
        version_code: String(v.version_code),
        min_required_version: v.min_required_version,
        min_required_version_code: String(v.min_required_version_code),
        is_active: Boolean(v.is_active),
        is_force_update: Boolean(v.is_force_update),
        store_url_android: v.store_url_android ?? "",
        store_url_ios: v.store_url_ios ?? "",
        release_title: v.release_title ?? "",
        release_notes: v.release_notes ?? "",
        force_update_message: v.force_update_message ?? "",
        optional_update_message: v.optional_update_message ?? "",
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (formData.is_active) {
        ...
        const overlappingActive = existing.filter(
          (v) =>
            v.id !== Number(id) &&
            v.is_active &&
            platformsOverlap(v.platform, formData.platform as Platform),
        )
      ...
      await apiFetch(`/super-admin/app-versions/${id}`, { method: "PUT", body })
  ...
  return (
    <div className="space-y-8 max-w-4xl">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-3xl font-bold">Edit App Version</h1>
          <p className="text-muted-foreground mt-2">Update app version metadata and update behavior.</p>
        </div>
      </div>
