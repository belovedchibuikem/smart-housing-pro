...
export async function POST(request: NextRequest) {
  try {
    ...
    const res = await fetch(`${API_BASE_URL}/admin/bulk/wallet-transfers/upload`, {
      method: 'POST',
      headers: {
    ...
    if (!res.ok) {
      ...
      return NextResponse.json(
        {
          success: false,
          message: errorData.message || 'Failed to upload wallet transfers',
    ...
    console.error('Bulk wallet transfer upload error:', error)
    return NextResponse.json(
