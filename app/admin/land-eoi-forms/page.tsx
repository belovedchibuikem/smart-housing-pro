...
export default function LandEOIFormsPage() {
  ...
  const fetchEOIForms = async () => {
    try {
      ...
      toast({
        title: "Error",
        description: "Failed to fetch land EOI forms",
        variant: "destructive",
      })
    } finally {
      setLoading(false)
  ...
  const handleViewForm = (id: string) => {
    router.push(`/admin/land-eoi-forms/${id}`)
  }
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold">Land Expression of Interest</h1>
        <p className="text-muted-foreground">Review and manage land parcel interest submissions</p>
      </div>
        ...
        <CardContent>
          <div className="flex gap-4 mb-6">
            <div className="flex-1 relative">
              ...
              <Input
                placeholder="Search by member name or land title..."
                className="pl-10"
                value={searchQuery}
          ...
          {loading ? (
            <div className="text-center py-8">
              ...
              <p className="text-muted-foreground">Loading land EOI forms...</p>
            </div>
          ) : eoiForms.length === 0 ? (
            <div className="text-center py-8 text-muted-foreground">No land EOI forms found</div>
          ) : (
            <div className="space-y-4">
              {eoiForms.map((form) => (
                <div
                  key={form.id}
                  className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between p-4 border rounded-lg hover:bg-muted/50"
                >
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <div className="text-sm text-muted-foreground">
                        ...
                        {form.land?.land_title || form.land?.land_code || "—"}
                      </div>
                      <div className="text-xs text-muted-foreground mt-1">
                        Submitted: {new Date(form.created_at).toLocaleDateString()} • Type:{" "}
                        {form.interest_type || "N/A"}
                      </div>
                    </div>
                  </div>
