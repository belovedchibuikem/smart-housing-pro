	...
	const handleUpload = async () => {
		...
		try {
			...
			if (!response.ok) {
				...
				let errorDescription = result.message || 'Failed to upload internal mortgage repayments'
				...
				if (errorType === 'file_validation') {
					...
					errorDescription = 'All internal mortgage repayment records failed to process. Please check the error details below.'
				} else if (errorType === 'empty_data') {
					errorTitle = 'Empty File'
					errorDescription = 'The file contains no valid internal mortgage repayment data.'
				}
			...
			const errorMessage = error.message || "Failed to upload internal mortgage repayments. Please try again."
